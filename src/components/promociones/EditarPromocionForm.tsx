...
// Estructura que viene de la DB (según tu include de Prisma)
type ItemPromocionDB = {
    producto: Producto;
    cantidad: number;
};

// Estructura completa de la promoción para editar
type PromocionData = {
    id: number;
    nombre: string;
    descripcion: string;
...
// Tipo para el estado local (igual que en Nueva Promoción)
...
export default function EditarPromocionForm({ promocion, actualizarAction, eliminarAction }: Props) {
    ...
    // --- INICIALIZACIÓN DEL ESTADO CON DATOS EXISTENTES ---
    // Nota: Como la DB no guarda "precioPromoUnitario" individualmente (solo el total de la promo),
    // inicializamos el precio unitario con el precio de lista del producto para referencia visual.
    const [productosSeleccionados, setProductosSeleccionados] = useState<ProductoSeleccionado[]>(
        promocion.productos.map(item => ({
            producto: item.producto,
            cantidad: item.cantidad,
            precioPromoUnitario: item.producto.precio // Valor inicial referencia
        }))
    );

    // Calculamos el total inicial basado en lo que hay guardado en la promo, no en la suma de productos
    // Sin embargo, para la lógica dinámica, usaremos la suma calculada.
    ...
    // --- LÓGICA DE INTERACCIÓN (Idéntica a Nueva Promoción) ---
    ...
    return (
        <div className="bg-white dark:bg-[#1E293B] shadow-sm rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
            ...
            <form action={handleSubmit} className="p-6 md:p-8">
                ...
                />
                {/* Enviamos el precio calculado automáticamente */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div className="col-span-1">
                        ...
                        {erroresCliente.nombre && (
                            <p className="text-xs text-red-500 mt-1 flex items-center"><span className="material-symbols-outlined text-xs mr-1">info</span>{erroresCliente.nombre}</p>
                        )}
                    </div>
                    ...
                    <div className="col-span-1">
                        ...
                        {erroresCliente.descripcion && (
                            <p className="text-xs text-red-500 mt-1 flex items-center"><span className="material-symbols-outlined text-xs mr-1">info</span>{erroresCliente.descripcion}</p>
                        )}
                    </div>
                    ...
                    <div className="col-span-1 md:col-span-2 relative" ref={wrapperRef}>
                        ...
                        {erroresCliente.productos && (
                            <p className="text-xs text-red-500 mb-2 flex items-center"><span className="material-symbols-outlined text-xs mr-1">info</span>{erroresCliente.productos}</p>
                        )}
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <div className="flex flex-wrap items-center gap-2 mt-1">
                                                    ...
                                                    <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-1.5 py-0.5 rounded flex items-center">
                                                        ...
                                                        Promo: {formatCurrency(item.precioPromoUnitario)}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between xl:justify-end gap-6 w-full xl:w-auto">
                                            ...
                                            <div className="flex flex-col items-center">
                                                ...
                                                <div className="flex items-center bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg overflow-hidden">
                                                    <button type="button" onClick={() => modificarCantidadBotones(item.producto.id, -1)} className="px-2 py-1.5 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors focus:outline-none">
                    ...
                    <div className="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div className="col-span-1">
                            ...
                            <p className="mt-1.5 text-[10px] text-slate-400 flex items-center">
                                <span className="material-symbols-outlined text-xs mr-1">info</span> Calculado automáticamente
                            </p>
                        </div>

                        {/* FECHA INICIO */}
                        <div className="col-span-1">
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Fecha de Inicio <span className="text-black dark:text-white">*</span></label>
                        ...
                        {/* FECHA FIN */}
                        <div className="col-span-1">
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Fecha de Fin <span className="text-black dark:text-white">*</span></label>
                            <div className={`flex items-center bg-slate-50 dark:bg-slate-800 border ${erroresCliente.fechas ? 'border-red-500' : 'border-slate-200 dark:border-slate-600'} rounded-lg p-1.5 focus-within:ring-2 focus-within:ring-slate-200 dark:focus-within:ring-slate-600 transition-all`}>
