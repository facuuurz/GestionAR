...
export default async function EditarPromocionPage(props: { params: Promise<{ id: string }> }) {
  ...
  // Esto crea versiones de las funciones que ya "saben" cuál es el ID de esta promoción
  const actualizarConId = actualizarPromocion.bind(null, promocion.id);
  const eliminarConId = eliminarPromocion.bind(null, promocion.id);
      ...
      <main className="flex-1 flex flex-col items-center py-8 px-4 sm:px-6 lg:px-8 w-full">
        <div className="w-full max-w-7xl">
            ...
            <div className="mb-8">
                ...
                <p className="mt-2 text-slate-500 dark:text-slate-400 text-lg">Modifique los detalles de la promoción vigente o elimínela.</p>
            </div>

            {/* 3. Renderizar Formulario Pasando las Props Faltantes */}
            <EditarPromocionForm 
                promocion={promocion} 
                actualizarAction={actualizarConId} // ✅ Soluciona el error
                eliminarAction={eliminarConId}     // ✅ Soluciona el error
            />
